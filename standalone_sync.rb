...
# OPTIMIZED VERSION v1.2 - Sequential with cached labour rates
...
class StandaloneLineItemSync
  ...
  def initialize
    ...
    log_info "Optimizations: Cached labour rates, No sleep delays"
    log_info "=" * 100
  end
  ...
  def run
    ...
    # Process deals sequentially (optimized with cached labour rates, no sleep)
    deals.each_with_index do |deal, index|
      ...
      log_progress_report(actual_index + 1, start_from) if (actual_index + 1) % 50 == 0
    end
    ...
    if quote["Sections"] && quote["Sections"].any?
      quote["Sections"].each do |section|
        ...
        section["CostCenters"]&.each do |cost_center|
          line_items.concat(process_cost_center(cost_center, section_id, section_name, @labour_rates_cache))
        end
      end
    end
  ...
  def process_cost_center(cost_center, section_id, section_name, labour_rates_cache)
    line_items = []
    ...
    # Process all item types - pass section and cost center IDs
    line_items.concat(process_items(items["Catalogs"], "Catalogue", "Catalog", section_id, section_name, cost_center_id, cost_center_name, cost_center_type, cost_center_description, primary_optional, labour_rates_cache))
    line_items.concat(process_items(items["OneOffs"], "One-Off", "OneOff", section_id, section_name, cost_center_id, cost_center_name, cost_center_type, cost_center_description, primary_optional, labour_rates_cache))
    line_items.concat(process_items(items["Prebuilds"], "Pre-Builds", "Prebuild", section_id, section_name, cost_center_id, cost_center_name, cost_center_type, cost_center_description, primary_optional, labour_rates_cache))
    line_items.concat(process_items(items["ServiceFees"], "Service", "ServiceFee", section_id, section_name, cost_center_id, cost_center_name, cost_center_type, cost_center_description, primary_optional, labour_rates_cache))
    line_items.concat(process_items(items["Labors"], "Labour", "LaborType", section_id, section_name, cost_center_id, cost_center_name, cost_center_type, cost_center_description, primary_optional, labour_rates_cache))
    
    line_items
  end
  
  def process_items(items, type, key, section_id, section_name, cost_center_id, cost_center_name, cost_center_type, cost_center_description, primary_optional, labour_rates_cache)
    ...
    items.map do |item|
      ...
      build_line_item(item, type, key, section_id, section_name, cost_center_id, cost_center_name, cost_center_type, cost_center_description, primary_optional, labour_rates_cache)
    end.compact
  end
  
  def build_line_item(item, type, key, section_id, section_name, cost_center_id, cost_center_name, cost_center_type, cost_center_description, primary_optional, labour_rates_cache)
    ...
    # Build properties array for HubSpot with ALL fields
    [
